...
class Result {
    ...
    public Result(int cls, Float output, Rect rect) {
        this.classIndex = cls;
        this.score = output*100;
        this.rect = rect;
    }

    public Result(int cls, Float output, Rect rect,Float distance) {
        this.classIndex = cls;
        this.score = output*100;
        this.rect = rect;
...
public class PrePostProcessor {
    ...
    static ArrayList<Result> outputsToNMSPredictions(float[] outputs, float imgScaleX, float imgScaleY, float ivScaleX, float ivScaleY, float startX, float startY, Frame frame) {
        ArrayList<Result> results = new ArrayList<>();
        for (int i = 0; i< mOutputRow; i++) {
            if (outputs[i* mOutputColumn +4] > mThreshold) {
                float x = outputs[i* mOutputColumn];
